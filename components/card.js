...
var Card = (function () {
  ...
  function cardHTML(art) {
    ...
    var posStyle = art.thumbnailPosition ? ' style="object-position:' + art.thumbnailPosition + '"' : '';
    var imgHTML = art.thumbnail
      ? '<img src="' + art.thumbnail + '" alt="" loading="lazy" decoding="async"' + posStyle + '>'
    ...
    return '<a class="article-card" href="article.html?id=' + art.id + '">' +
      ...
      '<div class="card-body">' +
        ...
        '<span class="card-content">' + (art.content || '').replace(/<[^>]+>/g, '').replace(/\$\$[\s\S]*?\$\$/g, '') + '</span>' +
