...
(function () {
  ...
  function renderArticle() {
    ...
    // 为文章内所有图片添加懒加载
    var imgs = document.getElementById('art-body').querySelectorAll('img');
    for (var i = 0; i < imgs.length; i++) {
      imgs[i].setAttribute('loading', 'lazy');
      imgs[i].setAttribute('decoding', 'async');
    }
    // 为视频添加静音 + 预加载控制
    var videos = document.getElementById('art-body').querySelectorAll('video');
    for (var j = 0; j < videos.length; j++) {
      videos[j].setAttribute('preload', 'none');
      videos[j].setAttribute('muted', '');
      videos[j].muted = true;
    }
